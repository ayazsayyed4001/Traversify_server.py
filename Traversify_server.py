...
def check_password(update, context):
    ...
    if user_password == password:
        ...
        update.message.reply_text("Welcome! to your home acces , following is your control panel,please make sure to get confirmation reciept before turning on/off devices  :",
...
def choices(update, context):
    ...
    if session_start_time is None or datetime.now() > session_start_time + timedelta(seconds=sessionLimit):
        ...
        return ConversationHandler.END
    if choice == 'Light on':
        update.message.reply_text("confirmation reciep:success, light on.")

        write(str(4))
        delay()
    elif choice == 'Light off':
        update.message.reply_text("confirmation reciept: success,light off .")

        write(str(4))
        delay()
    elif choice == 'Cam 1':
        update.message.reply_text("confirmation reciept: sending you pic in 5 seconds.")
        Takephoto(update,context)
    elif choice == 'Automode':
        update.message.reply_text("You enabled automatic mode.")
    elif choice == 'Fan on':
        update.message.reply_text("You turned the fan on.")
    elif choice == 'Fan off':
